...
### Phase 5: Add exec2 overload for simple await use case ✅ COMPLETE
...
1. **Add overloads to `SandboxEnvironment.exec2()`**
   - [x] Overload 1: `exec2(cmd, options, stream=True) -> Exec2Process` (default, existing behavior)
   - [x] Overload 2: `exec2(cmd, options, stream=False) -> Awaitable[ExecResult[str]]`
...
3. **Implementation for non-streaming mode**
   - [x] `_create_exec2_awaitable()` returns a coroutine
   - [x] `_exec2_await_impl()` submits job, polls until completion, accumulates output
...
**Files modified**:
- `src/inspect_ai/util/_sandbox/exec2.py`: Added `_build_shell_command()`, `_create_exec2_awaitable()`, `_exec2_await_impl()`
- `src/inspect_ai/util/_sandbox/environment.py`: Added overloads to `exec2()` method
